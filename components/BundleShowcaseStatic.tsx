...
interface BundleShowcaseStaticProps {
  bundles?: BundleItem[];
  limit?: number;
  mobileLimit?: number;
  desktopLimit?: number;
...
export default function BundleShowcaseStatic({
  bundles = BUNDLE_CATALOG,
  limit,
  mobileLimit,
  desktopLimit,
}: BundleShowcaseStaticProps) {
                  ...
                  <div className="mt-auto flex flex-col gap-3 border-t pt-4 sm:flex-row sm:items-center sm:justify-between">
                    <div>
                      {bundle.originalPrice > bundle.bundlePrice ? (
                        <p className="text-[10px] text-gray-500 line-through sm:text-xs">
