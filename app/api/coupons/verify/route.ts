...
export async function POST(request: Request) {
  try {
    ...
    if (!checkoutItems.length) {
      return NextResponse.json({ ok: false, error: 'Coupon preview-এর জন্য item দরকার।' }, { status: 400 });
    }

    const resolvedItems = resolveCartItems(checkoutItems);

    if (!resolvedItems.length) {
      return NextResponse.json({ ok: false, error: 'Valid item পাওয়া যায়নি।' }, { status: 400 });
    }
    ...
    if (!couponResult.coupon) {
      return NextResponse.json(
        { ok: false, error: couponResult.error || 'Coupon apply করা যায়নি।' },
        { status: 400 },
      );
    }
    ...
    return NextResponse.json(
      { ok: false, error: error instanceof Error ? error.message : 'Coupon verify failed' },
      { status: 500 },
    );
  }
}
